...
No local tools, no scripts, no runtime dependency: a manifest, an MCP registration, and a skill. It works in **Claude Code** and in **Claude Cowork** (installed plugins live at the account level and reload every session).
...
plugins/fastn/
  ...
  hooks/load-fastn-skill.sh           the session-start loader
  skills/fastn/SKILL.md               the fastn usage skill
```

## Configuration: the gateway endpoint

`plugins/fastn/.mcp.json` points at the fastn gateway:
...
/plugin install fastn@fastn-marketplace
...
Open the **Cowork** tab -> **Customize** -> add this marketplace (from GitHub) or **upload the plugin**, then install `fastn`. Restart the session so it loads. fastn then appears in the connector list, and the usage skill auto-triggers on integration, connector, and automation tasks.
